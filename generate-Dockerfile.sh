...
if [[ "$HELP" == 1 ]]; then
    ...
    echo "    --devel: Use the devel version of the base image (Warning: might cause problems when used with a different base image)"
    exit 21
fi
...
if [[ "use_devel_image" == 1 ]]; then
  echo "Using -devel base image"
  cat src/Dockerfile.header | sed 's/runtime-ubuntu/devel-ubuntu/g' >> $DOCKERFILE
else
